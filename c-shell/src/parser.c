...
static void free_command(Command *command){
    ...
    free_redirections(command->input_redirections,command->input_redirection_count);
...
void free_command_list(CommandList *command_list){
    ...
    if(command_list->pipelines!=NULL){
...
static int add_pipeline(CommandList *command_list){
    if(command_list->count>=command_list->capacity){
        ...
        Pipeline *new_pipelines=realloc(command_list->pipelines,new_capacity*sizeof(Pipeline));
    ...
    Pipeline *pipeline=&command_list->pipelines[command_list->count];
...
static int add_command(Pipeline *pipeline){
    if(pipeline->count>=pipeline->capacity){
        ...
        Command *new_commands=realloc(pipeline->commands,new_capacity*sizeof(Command));
...
static int add_argument(Command *command,const char *value){
    if(command->argc+1>=command->argv_capacity){
        ...
        char **new_argv=realloc(command->argv,new_capacity*sizeof(char *));
        if(new_argv==NULL){
            return 0;
        }
...
static int add_redirection(Redirection **redirections,size_t *count,size_t *capacity,RedirectionType type,const char *filename){
    if(*count>=*capacity){
        ...
        Redirection *new_redirections=realloc(*redirections,new_capacity*sizeof(Redirection));
...
static int add_input_redirection(Command *command,const char *filename){
    return add_redirection(&command->input_redirections,&command->input_redirection_count,&command->input_redirection_capacity,REDIR_INPUT,filename);
}
static int add_output_redirection(Command *command,RedirectionType type,const char *filename){
    return add_redirection(&command->output_redirections,&command->output_redirection_count,&command->output_redirection_capacity,type,filename);
}
...
static int parse_redirection(const TokenList *tokens,size_t *index,Command *command){
    ...
    const char *filename=tokens->tokens[*index+1].value;
    int success;
    if(operator==TOKEN_LT){
        success=add_input_redirection(command,filename);
    }else if(operator==TOKEN_GT){
        success=add_output_redirection(command,REDIR_OUTPUT,filename);
    }else{
        success=add_output_redirection(command,REDIR_APPEND,filename);
    }
    if(!success){
        return 0;
    }
...
static int parse_pipeline(const TokenList *tokens,size_t *index,Pipeline *pipeline){
    ...
    Command *current=&pipeline->commands[pipeline->count-1];
    while(*index<tokens->count){
        TokenType type=tokens->tokens[*index].type;
        if(type==TOKEN_WORD){
            if(!add_argument(current,tokens->tokens[*index].value)){
                return 0;
            }
            (*index)++;
            continue;
        ...
        if(is_redirection(type)){
            if(!parse_redirection(tokens,index,current)){
                return 0;
            }
            continue;
        }
        if(type==TOKEN_PIPE){
            ...
            (*index)++;
            if(*index>=tokens->count || tokens->tokens[*index].type!=TOKEN_WORD){
                return 0;
            }
            if(!add_command(pipeline)){
                return 0;
...
ParseResult parse_tokens(const TokenList *tokens,CommandList *command_list){
    ...
    size_t index=0;
    while(index<tokens->count){
        if(tokens->tokens[index].type==TOKEN_SEMI || tokens->tokens[index].type==TOKEN_AMP){
        ...
        Pipeline *pipeline=&command_list->pipelines[command_list->count-1];
        ...
        TokenType separator=tokens->tokens[index].type;
        index++;
        if(index>=tokens->count){
            if(separator==TOKEN_SEMI){
                free_command_list(command_list);
                return PARSE_INVALID_SYNTAX;
            }
            break;
