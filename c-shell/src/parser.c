...
static int add_pipeline(CommandList *command_list)
{
    if (command_list->count >= command_list->capacity) {
        ...
        Pipeline *new_pipelines =
            realloc(command_list->pipelines,
                           ...
                           const char *filename)
{
    if (*count >= *capacity) {
        ...
        Redirection *new_redirections =
            realloc(*redirections,
                             ...
                             Command *command)
{
    ...
    if (operator == TOKEN_LT) {
        success = add_input_redirection(command,
                                        filename);
    } else if (operator == TOKEN_GT) {
        success = add_output_redirection(command,
                                         REDIR_OUTPUT,
                                         filename);
    } else {
        success = add_output_redirection(command,
